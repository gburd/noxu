...
//! - Round-trip: from_bytes(data).get_data() == data.
...
fuzz_target!(|input: FuzzInput| {
    ...
    if let Some(got) = entry.get_data() {
    ...
    assert_eq!(
        entry_vec.get_data().map(|d| d.to_vec()),
        entry.get_data().map(|d| d.to_vec()),
        "from_vec != from_bytes"
    );
    ...
    assert_eq!(
        entry_data.get_data().map(|d| d.to_vec()),
        entry.get_data().map(|d| d.to_vec()),
    ...
    assert_eq!(
        entry_from_slice.get_data().map(|d| d.to_vec()),
        entry.get_data().map(|d| d.to_vec()),
        "From<&[u8]> mismatch"
    );
    ...
    assert_eq!(
        entry_from_vec.get_data().map(|d| d.to_vec()),
        entry.get_data().map(|d| d.to_vec()),
    ...
    let _ = entry_mut.get_data();
    let _ = entry_mut.data();

    // -- Partial configuration (should never panic) --
    let mut entry_partial = entry.clone();
    entry_partial.set_partial(input.partial_offset, input.partial_length, input.partial);
    assert_eq!(entry_partial.is_partial(), input.partial);
    assert_eq!(entry_partial.get_partial_offset(), input.partial_offset);
    assert_eq!(entry_partial.get_partial_length(), input.partial_length);

    // -- set_data round-trip --
    let mut entry_set = DatabaseEntry::new();
    assert!(entry_set.is_empty());
    entry_set.set_data(&input.data);
    if let Some(got) = entry_set.get_data() {
    ...
    if let Some(got) = entry_set_vec.get_data() {
        assert_eq!(got, input.data.as_slice(), "set_data_vec round-trip failed");
    }
    ...
    assert_eq!(entry_clear.get_data(), None);
    ...
    if let Ok(s) = std::str::from_utf8(&input.data) {
        ...
        assert_eq!(
            entry_from_str.get_data().unwrap(),
            input.data.as_slice(),
            "From<&str> mismatch"
        );
        let entry_from_string: DatabaseEntry = s.to_string().into();
        assert_eq!(
            entry_from_string.get_data().unwrap(),
            input.data.as_slice(),
