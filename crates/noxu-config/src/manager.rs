...
mod tests {
    ...
    fn test_override() {
        ...
        mgr.set("je.maxMemoryPercent", ParamValue::Int(75), false).unwrap();
    ...
    fn test_validation_rejects_out_of_range() {
        let mut mgr = ConfigManager::new();
        let result = mgr.set("je.maxMemoryPercent", ParamValue::Int(95), false);
    ...
    fn test_unknown_param() {
        let mut mgr = ConfigManager::new();
        let result = mgr.set("je.nonexistent", ParamValue::Bool(true), false);
    ...
    fn test_is_overridden() {
        let mut mgr = ConfigManager::new();
        assert!(!mgr.is_overridden("je.maxMemoryPercent"));
        mgr.set("je.maxMemoryPercent", ParamValue::Int(75), false).unwrap();
        assert!(mgr.is_overridden("je.maxMemoryPercent"));
    }
    ...
    fn test_get_definition() {
        let mgr = ConfigManager::new();
        let def = mgr.get_definition("je.maxMemoryPercent");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "je.maxMemoryPercent");
        assert!(mgr.get_definition("je.nonexistent").is_none());
    }
    ...
    fn test_set_get_roundtrip_all_types() {
        ...
        // Bool
        mgr.set("je.env.runCleaner", ParamValue::Bool(false), false).unwrap();
        assert!(!mgr.get_bool(&params::ENV_RUN_CLEANER));

        // Int
        mgr.set("je.maxMemoryPercent", ParamValue::Int(80), false).unwrap();
        assert_eq!(mgr.get_int(&params::MAX_MEMORY_PERCENT), 80);

        // Long
        mgr.set("je.maxMemory", ParamValue::Long(512 * 1024 * 1024), false).unwrap();
        assert_eq!(mgr.get_long(&params::MAX_MEMORY), 512 * 1024 * 1024);

        // Duration
        mgr.set("je.lock.timeout", ParamValue::Duration(Duration::from_secs(2)), false).unwrap();
    ...
    fn test_type_mismatch_rejected() {
        ...
        let result = mgr.set("je.env.recovery", ParamValue::Int(1), false);
        assert!(matches!(result, Err(ConfigError::TypeMismatch { .. })));

        // Int param given bool
        let result = mgr.set("je.maxMemoryPercent", ParamValue::Bool(true), false);
        assert!(matches!(result, Err(ConfigError::TypeMismatch { .. })));

        // Long param given int
        let result = mgr.set("je.maxMemory", ParamValue::Int(1024), false);
        assert!(matches!(result, Err(ConfigError::TypeMismatch { .. })));
    }
    ...
    fn test_int_out_of_range_min() {
        ...
        let result = mgr.set("je.maxMemoryPercent", ParamValue::Int(0), false);
    ...
    fn test_int_out_of_range_max() {
        ...
        let result = mgr.set("je.maxMemoryPercent", ParamValue::Int(91), false);
        assert!(matches!(result, Err(ConfigError::OutOfRange { .. })));
    }
    ...
    fn test_mutable_param_can_be_changed_when_open() {
        ...
        let result = mgr.set("je.maxMemoryPercent", ParamValue::Int(75), true);
        assert!(result.is_ok());
        assert_eq!(mgr.get_int(&params::MAX_MEMORY_PERCENT), 75);
    }
    ...
    fn test_default_fallback_when_not_overridden() {
        ...
        let val = mgr.get("je.maxMemoryPercent");
    ...
    fn test_get_returns_none_for_unknown() {
        let mgr = ConfigManager::new();
        assert!(mgr.get("je.nonexistent.param").is_none());
    }
}
