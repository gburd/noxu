...
impl PreparedLog {
    ...
    pub fn open(env: &Environment) -> Result<Self, crate::db::NoxuError> {
    ...
    pub fn record_prepare(&self, xid: &Xid) -> Result<(), crate::db::NoxuError> {
    ...
    pub fn remove(&self, xid: &Xid) -> Result<(), crate::db::NoxuError> {
        let key = Self::xid_to_key(xid);
        let k = DatabaseEntry::from_vec(key);
    ...
    pub fn recover_all(&self) -> Result<Vec<Xid>, crate::db::NoxuError> {
        use crate::db::{CursorConfig, Get};
...
mod tests {
    ...
    fn make_env() -> (Environment, TempDir) {
        ...
        let config = crate::db::EnvironmentConfig::new(dir.path().to_path_buf())
            .with_allow_create(true)
            .with_transactional(true);
    ...
    fn test_persist_across_reopen() {
        ...
        {
            let config =
                crate::db::EnvironmentConfig::new(dir.path().to_path_buf())
                    .with_allow_create(true)
                    .with_transactional(true);
            let env = Environment::open(config).unwrap();
            let log = PreparedLog::open(&env).unwrap();
