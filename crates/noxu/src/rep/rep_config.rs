...
use crate::rep::commit_durability::CommitDurability;
use crate::rep::consistency::ConsistencyPolicy;
use crate::rep::node_type::NodeType;
use crate::rep::quorum_policy::QuorumPolicy;
use crate::rep::rep_node::RepNode;
use crate::rep::stream::reconnect::ReconnectConfig;
...
///
/// Note: noxu-rep's [`crate::rep::net`] channel types are constructed
/// directly by the user code that drives the cluster (a `TcpListener`
/// on a port, a [`crate::rep::net::InMemoryTransport::new_group`] mesh,
...
pub enum RepTransportKind {
    /// Plaintext TCP via [`crate::rep::net::TcpChannel`].
    Tcp,
    /// TLS-encrypted TCP via `crate::rep::net::TlsTcpChannel`
    /// (requires `tls-rustls` or `tls-native`).
    Tls,
    /// QUIC over UDP via `crate::rep::net::QuicChannel`
    /// (requires the `quic` feature).
    Quic,
    /// In-process [`crate::rep::net::InMemoryTransport`].  Useful for
...
pub struct RepConfig {
    ...
    /// in-process [`crate::rep::net::InMemoryTransport`].  See
...
mod tests {
    use super::*;
    use crate::rep::commit_durability::ReplicaAckPolicy;
