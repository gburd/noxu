...
impl GroupService {
    ...
    pub fn update_node_vlsn(&self, name: &str, vlsn: u64) {
        ...
        let new_cbvlsn = nodes
            ...
            .filter(|n| {
                n.is_active
                    && n.node_type == crate::rep::node_type::NodeType::Electable
            })
            .map(|n| n.known_vlsn)
            .min()
            .unwrap_or(0);
    ...
    pub fn add_node(&self, info: NodeInfo) -> crate::rep::error::Result<()> {
        let mut nodes = self.nodes.write();
        if nodes.contains_key(&info.name) {
            return Err(crate::rep::error::RepError::NodeAlreadyExists(info.name));
        }
        log::info!(
    ...
    pub fn remove_node(&self, name: &str) -> crate::rep::error::Result<()> {
        let mut nodes = self.nodes.write();
        if nodes.remove(name).is_none() {
            return Err(crate::rep::error::RepError::NodeNotFound(name.to_string()));
        }
    ...
    pub fn update_node_status(
        ...
        match nodes.get_mut(name) {
            ...
            None => Err(crate::rep::error::RepError::NodeNotFound(name.to_string())),
        }
    }
    ...
    ///
    /// Returns an error if the named node does not exist.
    pub fn touch_node(&self, name: &str) -> crate::rep::error::Result<()> {
        let mut nodes = self.nodes.write();
        match nodes.get_mut(name) {
            Some(info) => {
