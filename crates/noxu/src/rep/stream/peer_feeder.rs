...
use crate::rep::net::service_dispatcher::ServiceHandler;
...
impl PeerLogScanner {
    ...
    /// by [`LogScanner::next_entry`](crate::rep::stream::feeder::LogScanner),
...
pub fn catch_up_from_peer(
    ...
    start_vlsn: u64,
    log_writer: &mut dyn crate::rep::stream::replica_stream::LogWriter,
) -> Result<bool> {
    use crate::rep::net::service_dispatcher::connect_to_service;
    use crate::rep::stream::replica_stream::ReplicaReceiver;
    use std::sync::Arc;
    use std::time::Duration;
...
impl MultiPeerCatchUp {
    ...
    where
        ...
        W: crate::rep::stream::replica_stream::LogWriter + Send + 'static,
    {
        use std::sync::atomic::{AtomicBool, Ordering};
        let make_writer = std::sync::Arc::new(make_writer);
        let done = std::sync::Arc::new(AtomicBool::new(false));
        let winner: std::sync::Arc<crate::sync::Mutex<Option<String>>> =
            std::sync::Arc::new(crate::sync::Mutex::new(None));
...
mod tests {
    ...
    fn test_group_service_cbvlsn_tracks_minimum() {
        use crate::rep::group_service::{GroupService, NodeInfo};
