...
use crate::rep::ack_tracker::AckTracker;
use crate::rep::elections::election_service::{
    ELECTION_SERVICE_NAME, ElectionAcceptorState, ElectionService,
};
use crate::rep::elections::master_tracker::MasterTracker;
use crate::rep::error::{RepError, Result};
use crate::rep::group_service::GroupService;
use crate::rep::master_transfer::MasterTransferConfig;
use crate::rep::net::service_dispatcher::TcpServiceDispatcher;
use crate::rep::network_restore::{NetworkRestore, NetworkRestoreConfig};
use crate::rep::network_restore_server::{
    NetworkRestoreServer, RESTORE_SERVICE_NAME,
};
use crate::rep::node_state::{NodeState, NodeStateMachine};
use crate::rep::rep_config::RepConfig;
use crate::rep::rep_stats::RepStats;
use crate::rep::state_change_listener::{StateChangeEvent, StateChangeListener};
use crate::rep::stream::feeder::Feeder;
use crate::rep::stream::peer_feeder::{
    PEER_FEEDER_SERVICE_NAME, PeerFeederService, PeerLogScanner,
};
use crate::rep::stream::replica_stream::{EnvironmentLogWriter, ReplicaStream};
use crate::rep::vlsn::vlsn_index::VlsnIndex;
use crate::rep::vlsn::vlsn_range::VlsnRange;
...
///
/// ```ignore
/// use crate::rep::{ReplicatedEnvironment, RepConfig};
///
...
impl ReplicatedEnvironment {
    ...
    pub fn new(config: RepConfig) -> Result<Self> {
        ...
        let vlsn_index = {
            ...
            if let Some(ref home) = config.env_home {
                match crate::rep::vlsn::persist::load_from_disk(home) {
                    Ok(Some(idx)) => {
                        log::info!(
                    ...
                    Err(e) => {
                        ...
                        let _ = std::fs::remove_file(
                            crate::rep::vlsn::persist::index_path(home),
                        );
    ...
    pub fn register_admin_service(self: &Arc<Self>) {
        if let Some(ref dispatcher) = self.tcp_dispatcher {
            crate::rep::group_admin::register_admin_service(
                dispatcher,
                Arc::downgrade(self),
            );
            log::debug!(
            ...
            .spawn(move || {
                    ...
                    && !me.is_shutdown()
                {
                    ...
                    let cap_lsn = me
                        ...
                        .unwrap_or(crate::util::NULL_LSN);
                    match crate::rep::vlsn::persist::flush_to_disk_capped(
                        &vlsn_index,
                        &home,
                        cap_lsn,
                    ) {
                ...
                let cap_lsn = me
                    ...
                    .map(|c| c.get_last_checkpoint_end())
                    .unwrap_or(crate::util::NULL_LSN);
                if let Err(e) = crate::rep::vlsn::persist::flush_to_disk_capped(
                    &vlsn_index,
                    &home,
                    cap_lsn,
                ) {
    ...
    fn run_election_loop(self: Arc<Self>) {
        ...
        loop {
            ...
            // remaining peers.
            let mut channels: Vec<Arc<dyn crate::rep::net::channel::Channel>> =
                Vec::new();
            for (peer_name, addr) in &peers {
                match crate::rep::net::service_dispatcher::connect_to_service(
                    *addr,
                    ELECTION_SERVICE_NAME,
                ) {
                    Ok(ch) => {
                        let arc: Arc<dyn crate::rep::net::channel::Channel> =
                            Arc::new(ch);
                        channels.push(arc);
                    }
                    Err(e) => {
            ...
            let outcome = crate::rep::elections::paxos::run_election(
                self_node_id,
                &self.config.node_name,
                &group,
                &channels,
    ...
    fn local_rep_group_with_self(&self) -> crate::rep::rep_group::RepGroup {
        ...
        if group.get_node(&self.config.node_name).is_none() {
            let mut self_node = crate::rep::rep_node::RepNode::new(
    ...
    pub fn with_environment(&self, env: Arc<EnvironmentImpl>) {
        ...
        if !env.recovery_vlsns.is_empty() {
            ...
            for &(vlsn, lsn_u64) in &env.recovery_vlsns {
                let lsn = crate::util::Lsn::from_u64(lsn_u64);
    ...
    pub fn add_peer(&self, node: crate::rep::rep_node::RepNode) -> Result<()> {
        use crate::rep::group_service::NodeInfo;
        ...
        // re-election.
        if self.is_master()
            && (node.node_type == crate::rep::node_type::NodeType::Electable
                || node.node_type == crate::rep::node_type::NodeType::Secondary)
        {
    ...
    pub fn update_peer_metadata(
        &self,
        name: &str,
        node: crate::rep::rep_node::RepNode,
    ) -> Result<()> {
    ...
    pub fn get_rep_group(&self) -> crate::rep::rep_group::RepGroup {
        use crate::rep::rep_group::RepGroup;
        ...
        for info in self.group_service.get_all_nodes() {
            let mut node = crate::rep::rep_node::RepNode::new(
                info.name.clone(),
                info.node_type,
                info.host.clone(),
                info.port,
    ...
    pub fn become_master(&self, term: u64) -> Result<()> {
        ...
        // gets a `Feeder` entry.
        {
            ...
            for peer in self.group_service.get_all_nodes() {
                ...
                if peer.node_type != crate::rep::node_type::NodeType::Electable
                    && peer.node_type != crate::rep::node_type::NodeType::Secondary
                {
    ...
    pub fn become_replica(&self, master_name: &str) -> Result<()> {
        ...
        self.replica_stream.set_state(
            crate::rep::stream::replica_stream::ReplicaStreamState::Connecting,
        );
        ...
        if let Some(env) = self.env_impl.lock().unwrap().clone() {
            if let Some(log_mgr) = env.get_log_manager() {
                let vlsn_index =
                    Arc::new(crate::rep::vlsn::vlsn_index::VlsnIndex::new(10));
                ...
                let handle = std::thread::Builder::new()
                    ...
                    .spawn(move || {
                        ...
                        loop {
                            ...
                            match crate::rep::stream::peer_feeder::catch_up_from_peer(
                                addr, 0, &mut writer,
                            ) {
                                Ok(true) => {
                                    log::info!(
    ...
    pub fn transfer_master(&self, config: MasterTransferConfig) -> Result<()> {
        ...
        let target_ack = crate::rep::group_admin::send_transfer_master(
            target_addr,
            &config.target_node,
            new_term,
        )
        ...
        for peer in self.group_service.get_all_nodes() {
            ...
            if let Ok(addr) = format!("{}:{}", peer.host, peer.port).parse() {
                let _ = crate::rep::group_admin::send_transfer_master(
                    addr,
                    &config.target_node,
                    new_term,
                );
    ...
    /// `crate::db::Environment` (i.e., before `with_environment` is
    ...
    pub fn close(&self) -> Result<()> {
        ...
        if let Some(ref home) = self.config.env_home
            && let Err(e) =
                crate::rep::vlsn::persist::flush_to_disk(&self.vlsn_index, home)
        {
            log::warn!(
    ...
    pub fn shutdown_group(
        ...
        for peer in self.group_service.get_all_nodes() {
            ...
            match crate::rep::group_admin::send_shutdown_group(addr) {
                Ok(true) => log::info!(
...
// ---------------------------------------------------------------------------
//
// `crate::db::Transaction::commit_with_durability` calls
...
impl ReplicaAckCoordinator for ReplicatedEnvironment {
    fn await_replica_acks(
        ...
        let electable_peers: u32 = group
            .get_nodes()
            .iter()
            .filter(|n| n.node_type == crate::rep::node_type::NodeType::Electable)
            .count() as u32;
    ...
    fn alloc_vlsn_for_recovered_commit(&self, lsn: crate::util::Lsn) -> u64 {
...
mod tests {
    ...
    fn test_state_change_listener_notification() {
        struct TestListener {
            ...
            last_new_state: crate::sync::Mutex<Option<NodeState>>,
        }
        ...
        let listener = Arc::new(TestListener {
            ...
            last_new_state: crate::sync::Mutex::new(None),
        });
    ...
    fn test_tcp_dispatcher_accepts_connection() {
        use crate::rep::net::Channel;
        use crate::rep::net::ServiceHandler;
        use crate::rep::net::service_dispatcher::connect_to_service;
        ...
        impl ServiceHandler for PingHandler {
            ...
            fn handle(&self, ch: Box<dyn Channel>) -> crate::rep::error::Result<()> {
