...
use crate::log::entry_type::{LOG_VERSION, LogEntryType};
use crate::log::error::{NoxuLogError, Result};
use crate::log::provisional::Provisional;
use byteorder::{LittleEndian, ReadBytesExt, WriteBytesExt};
use crate::util::{Lsn, VLSN_LOG_SIZE, Vlsn};
use std::io::Cursor;
...
impl LogEntryHeader {
    ...
    pub fn read_from_log(buf: &[u8], lsn: Lsn) -> Result<Self> {
        ...
        // Validate item size
        if (item_size as usize) > crate::log::MAX_ITEM_SIZE {
            // Sanity check: see crate::log::MAX_ITEM_SIZE.
    ...
    pub fn write_to_log(&self, buf: &mut Vec<u8>) -> Result<()> {
        ...
        if self.vlsn_present {
            ...
            buf.write_i64::<LittleEndian>(
                self.vlsn
                    .map_or(crate::util::NULL_VLSN_SEQUENCE, |v| v.sequence()),
            )?;
        }
...
mod tests {
    ...
    fn test_oversized_entry_rejected() {
        ...
        let big: u32 = (crate::log::MAX_ITEM_SIZE as u32) + 1;
