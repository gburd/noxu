...
use crate::recovery::checkpoint_end::CheckpointEnd;
use crate::recovery::checkpoint_start::CheckpointStart;
use crate::recovery::checkpoint_stat::CheckpointStats;
use crate::recovery::dirty_in_map::DirtyINMap;
use crate::recovery::error::{RecoveryError, Result};
use crate::cleaner::UtilizationTracker;
use crate::log::entry::FileSummaryLnEntry;
use crate::log::entry::bin_delta_log_entry::BinDeltaLogEntry;
use crate::log::entry::in_log_entry::InLogEntry;
use crate::log::{LogEntryType, LogManager, Provisional};
...
pub struct Checkpointer {
    ...
    cleaner: Option<Arc<crate::cleaner::Cleaner>>,
}
...
impl Checkpointer {
    ...
    pub fn new(config: CheckpointConfig) -> Self {
        Self {
            ...
            last_checkpoint_start: Mutex::new(crate::util::NULL_LSN),
            last_checkpoint_end: Mutex::new(crate::util::NULL_LSN),
    ...
    pub fn with_cleaner(mut self, cleaner: Arc<crate::cleaner::Cleaner>) -> Self {
    ...
    pub fn do_checkpoint(&self, invoker: &str) -> Result<CheckpointResult> {
        ...
        let end_lsn = if let Some(lm) = &self.log_manager {
            let ckpt_end = CheckpointEnd::new(
                ...
                // db_map.)
                crate::util::Lsn::new(0, 0), // first_active_lsn
                0,
                0,
                0,
                0,
    ...
    fn flush_upper_ins_internal(&self) -> Result<FlushResult> {
        ...
        for (level, node_arc) in &dirty_ins {
            ...
            let entry = InLogEntry::new(
                self.db_id,
                crate::util::NULL_LSN, // prev_full_lsn — no previous version tracking for upper INs yet
                crate::util::NULL_LSN, // prev_delta_lsn
                node_bytes,
            );
            let mut buf = bytes::BytesMut::with_capacity(entry.log_size());
            entry.write_to_log(&mut buf);
...
mod tests {
    ...
    fn test_checkpointer_new() {
        ...
        assert_eq!(checkpointer.get_last_checkpoint_end(), crate::util::NULL_LSN);
    }
    ...
    fn test_checkpointer_do_checkpoint() {
        ...
        assert!(result.start_lsn != crate::util::NULL_LSN);
        assert!(result.end_lsn != crate::util::NULL_LSN);
    ...
    fn test_is_checkpointed() {
        use crate::tree::tree::{BinStub, TreeNode};
        use parking_lot::RwLock as NodeRwLock;
        ...
        let bin = BinStub {
            ...
            dirty: false,
            is_delta: false,
            last_full_lsn: crate::util::NULL_LSN,
            last_delta_lsn: crate::util::NULL_LSN,
            generation: 0,
            parent: None,
    ...
    fn test_persist_file_summaries_writes_file_summary_ln_to_log() {
        use crate::cleaner::UtilizationTracker;
        use crate::log::{FileManager, LogEntryType, LogFileReader};
        use tempfile::TempDir;
