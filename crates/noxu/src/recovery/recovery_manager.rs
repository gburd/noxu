...
use crate::recovery::analysis_result::AnalysisResult;
use crate::recovery::dirty_in_map::{CheckpointReference, DirtyINMap};
use crate::recovery::error::Result;
use crate::recovery::log_scanner::{LnOperation, LnRecord, LogEntry, LogScanner};
use crate::recovery::recovery_info::RecoveryInfo;
use crate::recovery::rollback_tracker::RollbackTracker;
...
impl RecoveryManager {
    ...
    pub fn recover(
        ...
        mut tree: Option<&mut crate::tree::Tree>,
        use_checkpoint: bool,
    ) -> Result<RecoveryInfo> {
    ...
    pub fn recover_all(
        &mut self,
        scanner: &mut dyn LogScanner,
        trees: &mut HashMap<u64, crate::tree::Tree>,
        use_checkpoint: bool,
    ) -> Result<RecoveryInfo> {
        self.use_existing_checkpoint = use_checkpoint;
        ...
        for (_lsn, rec) in &self.redo_entries {
            ...
            let tree = trees
                .entry(rec.db_id)
                .or_insert_with(|| crate::tree::Tree::new(rec.db_id, 256));
    ...
    pub(crate) fn run_mapping_tree_undo_pass(
        &mut self,
        analysis: &mut crate::recovery::analysis_result::AnalysisResult,
    ) {
            ...
            match pe.entry {
                ...
                LogEntry::TxnPrepare(rec) => {
                    ...
                    result.record_prepare(
                        crate::recovery::analysis_result::PreparedTxnInfo {
                            txn_id: rec.txn_id,
                            prepare_lsn: rec.lsn,
    ...
    fn collect_prepared_txn_lns(
        &self,
        analysis: &AnalysisResult,
    ) -> hashbrown::HashMap<u64, Vec<crate::recovery::analysis_result::PreparedLnReplay>>
    {
        use crate::recovery::analysis_result::{PreparedLnOperation, PreparedLnReplay};
    ...
    fn redo_ln(tree: &mut crate::tree::Tree, rec: &LnRecord, lsn: Lsn) {
...
mod tests {
    use super::*;
    use crate::recovery::dirty_in_map::CheckpointReference;
    use crate::recovery::log_scanner::{
    ...
    fn test_x14_recovered_vlsns_populated() {
        ...
        trees.insert(1u64, crate::tree::Tree::new(1, 256));
        let mut mgr = RecoveryManager::new();
    ...
    fn test_c6_mapping_tree_undo_removes_aborted_namelns() {
        let mut analysis = crate::recovery::analysis_result::AnalysisResult::new();
        ...
        analysis.committed_txns.insert(10, crate::util::Lsn::new(0, 100));
    ...
    fn test_c6_aborted_db_creation_not_recovered() {
        ...
        scanner.push(
            lsn(0, 100),
            LogEntry::NameLn(crate::recovery::log_scanner::NameLnRecord {
