...
use crate::db::durability::{Durability, SyncPolicy};
use crate::db::environment::ActiveTxns;
...
pub struct Transaction {
    ...
    /// `crate::rep::ReplicatedEnvironment`).  When `Some`, a successful
...
impl Transaction {
    ...
    /// [`Environment::begin_transaction`][crate::db::environment::Environment::begin_transaction]
    ...
    ///
    /// **Internal** — `crate::txn::Txn` is not re-exported by `noxu-db`.
    ...
    pub fn commit_with_durability(&self, durability: Durability) -> Result<()> {
        ...
        let ack_err: Option<NoxuError> = if !self.read_only
            && durability.replica_ack
                != crate::db::durability::ReplicaAckPolicy::None
        ...
        if let Some(inner) = &self.inner_txn {
            ...
            if let Some(env) = &self.env_impl {
                ...
                let mut db_handles: HashMap<
                    i64,
                    Arc<crate::sync::RwLock<crate::dbi::DatabaseImpl>>,
                > = HashMap::new();
    ...
    pub fn prepare(
        ...
        let (first_lsn, last_lsn) = match &self.inner_txn {
            ...
            None => {
                (crate::util::NULL_LSN.as_u64(), crate::util::NULL_LSN.as_u64())
            }
        };
        ...
        if !self.read_only
            && let Some(lm) = &self.log_manager
            && first_lsn != crate::util::NULL_LSN.as_u64()
        {
        ...
        if let Some(inner) = &self.inner_txn {
            ...
            if let Some(env) = &self.env_impl {
                ...
                for undo in undo_records {
                    let Some(abort_key) = undo.abort_key else { continue };
                    let db_id =
                        crate::dbi::DatabaseId::new(undo.database_id as i64);
    ...
    fn write_txn_prepare(
        ...
        use crate::log::{LogEntryType, Provisional, entry::TxnPrepareEntry};
