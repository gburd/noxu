...
use crate::db::cursor_config::CursorConfig;
use crate::db::database_config::DatabaseConfig;
use crate::db::database_entry::DatabaseEntry;
use crate::db::database_stats::{BtreeStats, DatabaseStats};
use crate::db::error::{NoxuError, Result};
use crate::db::join_config::JoinConfig;
use crate::db::join_cursor::JoinCursor;
use crate::db::lock_mode::LockMode;
use crate::db::operation_status::OperationStatus;
use crate::db::preload::{PreloadConfig, PreloadStats};
use crate::db::read_options::ReadOptions;
use crate::db::secondary_cursor::SecondaryCursor;
use crate::db::sequence::Sequence;
use crate::db::sequence_config::SequenceConfig;
use crate::db::stats_config::StatsConfig;
...
use crate::txn::{Durability, LockManager, Txn, TxnManager, UndoRecord};
...
/// # Example
/// ```ignore
/// use crate::db::{Environment, EnvironmentConfig, DatabaseConfig, DatabaseEntry};
...
pub struct Database {
    ...
    cleaner_throttle: Option<Arc<crate::cleaner::CleanerThrottle>>,
    ...
    pub(crate) secondaries: Arc<
        RwLock<
            Vec<
                std::sync::Weak<
                    dyn crate::db::secondary_database::SecondaryHook + Send + Sync,
                >,
            >,
        >,
    >,
    ...
    pub(crate) fk_referrers: Arc<
        RwLock<
            Vec<
                std::sync::Weak<
                    dyn crate::db::secondary_database::FkReferrer + Send + Sync,
                >,
            >,
        >,
    >,
...
impl Database {
    ...
    fn apply_auto_txn_undo(&self, mut undo_records: Vec<UndoRecord>) {
        ...
        for undo in undo_records {
            ...
            if undo.abort_known_deleted {
                ...
                let lsn = crate::util::Lsn::from_u64(undo.abort_lsn);
    ...
    pub fn put(
        ...
        let data_bytes: &[u8] = if data.is_partial() {
            ...
            let existing = {
                ...
                match tmp_cursor
                    .search(key_bytes, None, crate::dbi::SearchMode::Set)
                    .map_err(|e| {
                        NoxuError::OperationNotAllowed(e.to_string())
                    })? {
                    crate::dbi::OperationStatus::Success => {
    ...
    pub fn put_with_options(
        ...
        if opts.ttl > 0 && result == OperationStatus::Success {
            let expiration_hours =
                crate::util::current_time_hours().saturating_add(opts.ttl as u32);
            self.db_impl
                .read()
    ...
    pub fn delete(
        ...
        let mut run_delete = |cursor: &mut CursorImpl| -> Result<bool> {
            ...
            while let crate::dbi::OperationStatus::Success = cursor
                .search(key_bytes, None, SearchMode::Set)
                .map_err(|e| NoxuError::OperationNotAllowed(e.to_string()))?
            {
                if track_old_data {
    ...
    ///
    /// ```no_run
    /// # use crate::db::{Database, DatabaseConfig, DatabaseEntry,
    /// #              Environment, EnvironmentConfig};
    /// # use std::path::PathBuf;
    /// # fn main() -> crate::db::Result<()> {
    ...
    pub fn iter(
        &self,
        txn: Option<&Transaction>,
    ) -> Result<crate::db::db_iter::DbIter> {
        let cursor = self.open_cursor(txn, None)?;
        Ok(crate::db::db_iter::DbIter::new(cursor))
    }
    ...
    pub fn range<K: AsRef<[u8]>>(
    ...
    ) -> Result<crate::db::db_iter::DbRange> {
        use std::ops::Bound;
        ...
        let cursor = self.open_cursor(txn, None)?;
        Ok(crate::db::db_iter::DbRange::new(cursor, start, end))
    }
    ///
    ///
    ///
    ...
    pub fn close(&self) -> Result<()> {
        ...
        let _ = self
            .env_impl
            .lock()
            .close_database(crate::dbi::DatabaseId::new(self.id as i64));
        Ok(())
    }
    ...
    pub(crate) fn live_secondaries(
        &self,
    ) -> Vec<Arc<dyn crate::db::secondary_database::SecondaryHook + Send + Sync>>
    {
    ...
    pub(crate) fn live_fk_referrers(
        &self,
    ) -> Vec<Arc<dyn crate::db::secondary_database::FkReferrer + Send + Sync>> {
    ...
    pub fn scan_all_kv(&self) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
        ...
        if first_status != crate::dbi::OperationStatus::Success {
            return Ok(Vec::new());
        }
        ...
        loop {
            ...
            let status = cursor
                ...
                .map_err(|e| NoxuError::OperationNotAllowed(e.to_string()))?;
            if status != crate::dbi::OperationStatus::Success {
                break;
            }
        }
    ...
    pub fn verify(
        &self,
        config: &crate::engine::VerifyConfig,
    ) -> Result<crate::engine::VerifyResult> {
        self.check_open()?;
        let guard = self.db_impl.read();
        Ok(crate::engine::verify_database_impl(&guard, config))
    }
    ...
    fn check_open(&self) -> Result<()> {
        ...
        if self.env_invalid.load(Ordering::Acquire) {
            return Err(NoxuError::environment_with_reason(
                crate::db::error::EnvironmentFailureReason::UnexpectedStateFatal,
        ...
        {
            return Err(NoxuError::environment_with_reason(
                crate::db::error::EnvironmentFailureReason::LogWrite,
    ...
    /// [`crate::db::disk_ordered_cursor::open_disk_ordered_cursor_multi`].
    /// Returns `None` for non-WAL environments.
    pub(crate) fn cached_log_manager(
        &self,
    ) -> Option<&std::sync::Arc<crate::log::LogManager>> {
    ...
    /// cursor producer.
    pub(crate) fn database_id_for_doc(&self) -> crate::dbi::DatabaseId {
        crate::dbi::DatabaseId::new(self.id as i64)
    }
...
mod tests {
    ...
    fn test_get_search_map_err_via_hook() {
        ...
        crate::dbi::set_cursor_fail_after(1); // fail on the 1st check_state (search)
        let key = DatabaseEntry::from_bytes(b"any");
        let mut data = DatabaseEntry::new();
        let result = db.get(None, &key, &mut data);
        crate::dbi::clear_cursor_fail_flag();
        assert!(result.is_err());
    }
    ...
    fn test_get_get_current_map_err_via_hook() {
        ...
        crate::dbi::set_cursor_fail_after(2);
        let key = DatabaseEntry::from_bytes(b"k");
        let mut data = DatabaseEntry::new();
        let result = db.get(None, &key, &mut data);
        crate::dbi::clear_cursor_fail_flag();
    ...
    fn test_put_map_err_via_hook() {
        let (_tmp, _env, db) = temp_env_and_db();
        crate::dbi::set_cursor_fail_after(1);
        let key = DatabaseEntry::from_bytes(b"k");
        let val = DatabaseEntry::from_bytes(b"v");
    ...
    fn test_scan_all_kv_retrieve_next_map_err_via_hook() {
        ...
        crate::dbi::set_cursor_fail_after(3);
        let result = db.scan_all_kv();
        crate::dbi::clear_cursor_fail_flag();
        assert!(result.is_err());
    }
    ...
    fn test_x13_env_invalid_blocks_cursor_get() {
        ...
        let mut out = DatabaseEntry::new();
        let result =
            cursor.get(&mut key, &mut out, crate::db::get::Get::First, None);
        assert!(
            matches!(result, Err(NoxuError::EnvironmentFailure { .. })),
