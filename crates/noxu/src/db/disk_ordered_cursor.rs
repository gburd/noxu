...
use crate::dbi::DiskOrderedCursorImpl;
...
///
/// ```ignore
/// use crate::db::{DatabaseEntry, DiskOrderedCursorConfig, OperationStatus};
///
/// # fn example(db: &crate::db::Database) -> crate::db::Result<()> {
...
impl Database {
    ...
    pub fn open_disk_ordered_cursor(
        ...
        let inner = DiskOrderedCursorImpl::open(
            ...
            crate::dbi::DiskOrderedCursorOptions {
...
/// databases.
///
/// All databases must belong to the same [`crate::db::Environment`].  The cursor
