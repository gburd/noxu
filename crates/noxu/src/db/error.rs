...
impl From<crate::dbi::DbiError> for NoxuError {
    fn from(e: crate::dbi::DbiError) -> Self {
        use crate::dbi::DbiError;
        match e {
...
impl From<crate::txn::TxnError> for NoxuError {
...
mod tests {
    ...
    fn test_from_dbi_error() {
        let dbi_err = crate::dbi::DbiError::CursorClosed;
        let err: NoxuError = NoxuError::from(dbi_err);
        assert!(matches!(err, NoxuError::CursorClosed));

        let e: NoxuError =
            crate::dbi::DbiError::DatabaseNotFound("x".into()).into();
        assert!(matches!(e, NoxuError::DatabaseNotFound(_)));

        let e: NoxuError =
            crate::dbi::DbiError::EnvironmentFailure { reason: "disk".into() }
                .into();
    ...
    fn test_from_txn_error() {
        use crate::txn::{LockType, TxnError};
