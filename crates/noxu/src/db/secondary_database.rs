...
use crate::db::secondary_config::SecondaryConfig;
use crate::db::secondary_cursor::SecondaryCursor;
use crate::db::transaction::Transaction;
use crate::dbi::{CursorImpl, GetMode};
...
impl SecondaryHookState {
    ...
    fn insert_sec_key(
        ...
        let status =
            cursor
                .put(sec_key, pri_key, crate::db::put::Put::NoDupData)
...
impl FkReferrer for SecondaryHookState {
    ...
    fn on_foreign_key_deleted(
        ...
        match self.config.foreign_key_delete_action {
            crate::db::secondary_config::ForeignKeyDeleteAction::Abort => {
            ...
            crate::db::secondary_config::ForeignKeyDeleteAction::Cascade => {
                ...
                let child_pris: Vec<DatabaseEntry> = {
                    ...
                    let mut st = cursor
                        .get(&mut sk, &mut pk, crate::db::get::Get::Search, None)
                        .map_err(|e| {
                            NoxuError::OperationNotAllowed(e.to_string())
                        })?;
                    while st == OperationStatus::Success {
                        ...
                        st = cursor
                            .get(&mut sk, &mut pk, crate::db::get::Get::Next, None)
                            .map_err(|e| {
                                NoxuError::OperationNotAllowed(e.to_string())
                            })?;
                    }
            ...
            crate::db::secondary_config::ForeignKeyDeleteAction::Nullify => {
...
/// # Example
/// ```ignore
/// use crate::db::{Database, DatabaseEntry};
/// use crate::db::secondary_config::{SecondaryConfig, SecondaryKeyCreator};
/// use crate::db::secondary_database::SecondaryDatabase;
///
/// struct MyKeyCreator;
...
impl SecondaryDatabase {
    ...
    pub fn truncate(&self) -> Result<u64> {
        ...
        if cursor.get(&mut sec_key, &mut data, crate::db::get::Get::First, None)?
    ...
    fn populate_from_primary_scan(&self, primary: &Database) -> Result<()> {
        ...
        while first_status == crate::dbi::OperationStatus::Success {
            let (k, v) = cursor
                .get_current()
                .map_err(|e| NoxuError::OperationNotAllowed(e.to_string()))?;
