...
use crate::db::checkpoint_config::CheckpointConfig;
use crate::db::database::Database;
use crate::db::database_config::DatabaseConfig;
use crate::db::environment_config::EnvironmentConfig;
use crate::db::environment_mutable_config::EnvironmentMutableConfig;
use crate::db::error::{NoxuError, Result};
use crate::db::transaction::Transaction;
use crate::db::transaction_config::TransactionConfig;
use hashbrown::HashMap;
use crate::dbi::{DbiEnvConfig, EnvironmentImpl};
use crate::engine::EnvironmentStats;
use crate::engine::env_stats::{
...
/// # Example
/// ```ignore
/// use crate::db::{Environment, EnvironmentConfig};
...
pub struct Environment {
    ...
    last_checkpoint_end_lsn: Mutex<crate::util::Lsn>,
    /// Optional replica-ack coordinator (typically a
    /// `crate::rep::ReplicatedEnvironment`).  When set via
    ...
    replica_coordinator: Mutex<Option<crate::dbi::SharedReplicaAckCoordinator>>,
    /// Per-commit timeout for replica acknowledgments.  Mirrors
    /// `crate::rep::RepConfig::replica_ack_timeout`; defaults to 5s.
...
impl Environment {
    ...
    pub fn open(config: EnvironmentConfig) -> Result<Self> {
        ...
        Ok(Environment {
            ...
            last_checkpoint_end_lsn: Mutex::new(crate::util::NULL_LSN),
    ...
    pub fn open_database(
        ...
        let mut dbi_config = crate::dbi::DatabaseConfig::new();
        ...
        let db_impl_arc = {
            ...
            .map_err(|e| {
                match &e {
                    crate::dbi::DbiError::DatabaseNotFound(_) => {
        ...
        let mut databases = self.databases.lock();
        {
            ...
            env_impl.rename_database(old_name, new_name).map_err(
                |e| match &e {
                    ...
                    crate::dbi::DbiError::DatabaseAlreadyExists(_) => {
    ...
    pub fn begin_transaction(
        ...
        if config.is_none() {
            ...
            let derived = match (
                ...
                (true, _) => {
                    Some(crate::db::durability::Durability::COMMIT_NO_SYNC)
                }
                (_, true) => {
                    Some(crate::db::durability::Durability::COMMIT_WRITE_NO_SYNC)
                }
                _ => None,
            };
    ...
    ///
    /// `crate::rep::ReplicatedEnvironment` implements
    /// `crate::dbi::ReplicaAckCoordinator`; users typically wire it as:
    ///
    /// ```ignore
    ...
    pub fn set_replica_coordinator(
        &self,
        coord: crate::dbi::SharedReplicaAckCoordinator,
    ) {
    ...
    /// `crate::rep::RepConfig::replica_ack_timeout`.
    ...
    pub fn checkpoint(&self, config: Option<&CheckpointConfig>) -> Result<()> {
        ...
        // `crate::recovery::Checkpointer::do_checkpoint` is invoker-only.
    ...
    ///
    /// The XA layer (`crate::xa::XaEnvironment::xa_recover`) reads this
    ...
    pub fn apply_recovered_prepared_lns(
        &self,
        lns: &[crate::recovery::PreparedLnReplay],
    ) -> Result<()> {
        let env_impl = self.env_impl.lock();
        for ln in lns {
            let db_id = crate::dbi::DatabaseId::new(ln.db_id as i64);
            ...
            match ln.operation {
                crate::recovery::PreparedLnOperation::Insert
                | crate::recovery::PreparedLnOperation::Update => {
                ...
                crate::recovery::PreparedLnOperation::Delete => {
    ...
    pub fn verify(
        ...
        let mut merged = crate::engine::VerifyResult::new();
        for db_arc in &all_dbs {
            let guard = db_arc.read();
            let result = crate::engine::verify_database_impl(&guard, config);
    ...
    fn check_open(&self) -> Result<()> {
        ...
        if !self.env_valid.load(Ordering::Acquire) {
            return Err(NoxuError::environment_with_reason(
                crate::db::error::EnvironmentFailureReason::ForcedShutdown,
...
fn write_txn_end_for_recovered(
    ...
    fsync: bool,
    flush: bool,
) -> Result<crate::util::Lsn> {
    use bytes::BytesMut;
    use crate::log::{LogEntryType, Provisional, entry::TxnEndEntry};
...
mod tests {
    ...
    fn test_verify_environment_with_data_passes() {
        use crate::db::{DatabaseConfig, DatabaseEntry, VerifyConfig};
        let (_tmp, config) = temp_env_config();
        let env = Environment::open(config).unwrap();
    ...
    fn test_env_txn_no_sync_applies_to_explicit_txn() {
        ...
        assert_eq!(
            dur,
            crate::db::durability::Durability::COMMIT_NO_SYNC,
