...
use crate::dbi::database_impl::DatabaseImpl;
use crate::dbi::dbi_config::DbiEnvConfig;
use crate::dbi::file_manager_scanner::FileManagerLogScanner;
use crate::dbi::throughput_stats::ThroughputStatsSnapshot;
use crate::dbi::{
    DatabaseConfig, DatabaseId, DbType, DbiError, EnvState,
    EnvironmentFailureReason, NodeSequence,
};
use crate::cleaner::{Cleaner, UtilizationTracker, UtilizationTrackerObserver};
use crate::evictor::{Arbiter, EvictionSource, Evictor};
use crate::log::{
    FileManager, LogEntryType, LogManager, Provisional,
    entry::{LnLogEntry, TxnEndEntry},
};
use crate::recovery::RecoveryManager;
use crate::sync::Mutex as NoxuMutex;
use crate::txn::{LockManager, Txn, TxnManager};
use crate::util::{lsn::NULL_LSN, vlsn::NULL_VLSN};
...
pub struct EnvironmentImpl {
    ...
    recovered_trees: Mutex<HashMap<u64, crate::tree::Tree>>,
    ...
    /// XA layer (`crate::xa::XaEnvironment`) reads this via
    ...
    /// discards them.
    recovered_prepared_txns: Mutex<Vec<crate::recovery::PreparedTxnInfo>>,
    recovered_prepared_lns:
        Mutex<HashMap<u64, Vec<crate::recovery::PreparedLnReplay>>>,
    ...
    primary_tree: Arc<std::sync::RwLock<crate::tree::Tree>>,
    ...
    db_trees_registry: Arc<
        std::sync::Mutex<
            std::collections::HashMap<
                i64,
                Arc<std::sync::RwLock<crate::tree::Tree>>,
            >,
        >,
    >,
    ...
    ///
    ///
    checkpointer: Option<Arc<crate::recovery::checkpointer::Checkpointer>>,
    ...
    ///
    /// (extended fork).
    data_eraser: Mutex<crate::cleaner::DataEraser>,
    ...
    ///
    /// (extended fork).
    extinction_scanner: Mutex<crate::cleaner::ExtinctionScanner>,
    ...
    ///
    /// (extended fork).
    backup_manager: Mutex<crate::dbi::backup_manager::BackupManager>,
...
impl EnvironmentImpl {
    ...
    ///
    /// The caller (typically `crate::db::Environment::open`) translates its
    ...
    fn new_with_config_inner(
        ...
        let mut recovered: HashMap<u64, crate::tree::Tree> = HashMap::new();
        ...
        let mut recovered_prepared: Vec<crate::recovery::PreparedTxnInfo> =
            Vec::new();
        let mut recovered_prepared_lns: HashMap<
            u64,
            Vec<crate::recovery::PreparedLnReplay>,
        > = HashMap::new();
        ...
        let log_manager_and_tracker = if !read_only {
            ...
            let mut recovery_trees: HashMap<u64, crate::tree::Tree> =
                HashMap::new();
            recovery_trees.insert(1u64, crate::tree::Tree::new(1, 256));
            ...
            if let Ok(fm) = FileManager::new(
                ...
                let mut rmgr = crate::recovery::RecoveryManager::new();
                let mut recovery_trees: HashMap<u64, crate::tree::Tree> =
                    HashMap::new();
                recovery_trees.insert(1u64, crate::tree::Tree::new(1, 256));
                if let Ok(info) =
        ...
        let mut primary_tree_inner = crate::tree::Tree::new(1, 256);
        primary_tree_inner.set_memory_counter(Arc::clone(&cache_usage));
        let primary_tree: Arc<std::sync::RwLock<crate::tree::Tree>> =
        ...
        let off_heap_cache = Arc::new(crate::evictor::OffHeapCache::new(
        ...
        let checkpointer = log_manager.as_ref().map(|lm| {
            use crate::recovery::checkpointer::{CheckpointConfig, Checkpointer};
        ...
        let env = EnvironmentImpl {
            ...
            data_eraser: Mutex::new(crate::cleaner::DataEraser::new()),
            extinction_scanner: Mutex::new(
                crate::cleaner::ExtinctionScanner::new(),
            ),
            backup_manager: Mutex::new(
                crate::dbi::backup_manager::BackupManager::new(),
            ),
            utilization_tracker,
            cache_usage,
        };
        ...
        let count = {
                ...
                && let Some(tree) = db_guard.get_real_tree()
            {
                ...
                for node_arc in all_nodes {
                    ...
                    if let crate::tree::tree::TreeNode::Bottom(bin) = &*node_guard
                    {
            ...
            let new_tree =
                crate::tree::Tree::new(db_id.as_i64() as u64, max_entries);
    ...
    /// components.  The XA layer (`crate::xa::XaEnvironment::xa_recover`)
    ...
    pub fn recovered_prepared_txns(
        &self,
    ) -> Vec<crate::recovery::PreparedTxnInfo> {
    ...
    pub fn get_cleaner_throttle(
        &self,
    ) -> Option<Arc<crate::cleaner::CleanerThrottle>> {
    ...
    pub fn get_checkpointer(
        &self,
    ) -> Option<Arc<crate::recovery::checkpointer::Checkpointer>> {
    ...
    pub fn run_cleaner(
        ...
        n_files: u32,
        force: bool,
    ) -> Result<crate::cleaner::CleanResult, DbiError> {
        match &self.cleaner {
    ...
    pub fn discard_extinct_records(
        ...
        let task = crate::cleaner::ExtinctionTask {
    ...
    ///
    /// (extended fork).
    pub fn enqueue_erase(&self, request: crate::cleaner::EraseRequest) {
...
mod tests {
    ...
    fn test_recovery_on_reopen() {
        ...
        assert!(
            end_of_log.file_offset()
                > crate::log::file_header::FILE_HEADER_SIZE as u32,
            "end-of-log offset {:#x} should be past the file header ({})",
            end_of_log.file_offset(),
            crate::log::file_header::FILE_HEADER_SIZE
        );
    }
    ...
    fn test_write_after_reopen_does_not_overwrite() {
        ...
        use crate::dbi::file_manager_scanner::FileManagerLogScanner;
        use crate::log::FileManager;
        use crate::recovery::{LogEntry, LogScanner};
        use std::sync::Arc;
        ...
        let entries = scanner.scan_forward(crate::util::NULL_LSN, end);
