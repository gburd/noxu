...
use bytes::BytesMut;
use crate::log::{LogEntryType, LogManager, Provisional, entry::LnLogEntry};
use crate::tree::{BinEntry, Tree};
use crate::txn::{LockManager, LockType, Locker, Txn};

use crate::dbi::dup_key_data;
use crate::dbi::throughput_stats::ThroughputStats;
use crate::sync::RwLock;
use crate::util::{Lsn, vlsn::NULL_VLSN};
...
/// This implementation wires cursor traversal to `crate::tree::Tree`:
///
...
pub struct CursorImpl {
    ...
    current_bin_arc: Option<
        std::sync::Arc<crate::tree::NodeRwLock<crate::tree::tree::TreeNode>>,
    >,
...
impl CursorImpl {
    ...
    pub fn new(db_impl: Arc<RwLock<DatabaseImpl>>, locker_id: i64) -> Self {
        ...
        CursorImpl {
            ...
            current_key: None,
            current_data: None,
            current_lsn: crate::util::NULL_LSN.as_u64(),
            current_index: -1,
    ...
    fn get_slot_before_image(&self, key: &[u8]) -> (Option<Vec<u8>>, u64) {
        ...
        if let Some(tree) = db.get_real_tree() {
            match Self::get_data_from_tree(&tree, key) {
                ...
                None => (None, crate::util::NULL_LSN.as_u64()),
            }
        } else {
            (None, crate::util::NULL_LSN.as_u64())
        }
    }
    ...
    /// derived from `(db_id, key)` via [`crate::util::Lsn::synthetic_key_lock_id`].
    ...
    fn lock_write_before_log(
        ...
        let null = crate::util::NULL_LSN.as_u64();
    ...
    fn contest_synthetic_key_for_missing_read(
        ...
        if let Some(txn) = &self.txn_ref {
            ...
            match guard.lock(synthetic_lsn, LockType::Read, true) {
                ...
                Err(crate::txn::TxnError::LockNotAvailable { .. }) => {
    ...
    fn finalize_write_lock(
        ...
        if new_lsn_u64 == crate::util::NULL_LSN.as_u64() {
            return Ok(());
        }
        ...
        if let Some(txn) = &self.txn_ref {
            let db_id = self.db_impl.read().get_id().id() as u64;
            let mut guard = txn.lock().unwrap();
            if old_lsn != crate::util::NULL_LSN.as_u64() {
            ...
            let abort_known_deleted = old_lsn == crate::util::NULL_LSN.as_u64();
    ...
    fn lock_ln(&self, lsn: u64) -> Result<bool, DbiError> {
        if lsn == crate::util::NULL_LSN.as_u64() {
            return Ok(false);
        }
        if let Some(txn) = &self.txn_ref {
            let mut guard = txn.lock().unwrap();
    ...
    fn descend_to_bin(
        node: std::sync::Arc<crate::tree::NodeRwLock<crate::tree::tree::TreeNode>>,
    ) -> Option<std::sync::Arc<crate::tree::NodeRwLock<crate::tree::tree::TreeNode>>>
    {
        use crate::tree::tree::TreeNode;
    ...
    fn put_dup(
            ...
            PutMode::Overwrite => {
                ...
                let exists_old_lsn: u64 = {
                    ...
                    db.get_real_tree()
                        ...
                        .map(|(_, lsn)| lsn)
                        .unwrap_or(crate::util::NULL_LSN.as_u64())
                };
        ...
        let old_lsn = crate::util::NULL_LSN.as_u64();
    ...
    fn log_ln_write(
        ...
        if self.db_impl.read().is_deferred_write() {
            return Ok(crate::util::NULL_LSN);
        }

        let lm = match &self.log_manager {
            Some(lm) => lm,
            None => return Ok(crate::util::NULL_LSN),
        };
        ...
        let old_lsn_opt = if self.current_lsn != crate::util::NULL_LSN.as_u64() {
    ...
    pub fn delete(&mut self) -> Result<OperationStatus, DbiError> {
        ...
        self.current_data = None;
        self.current_lsn = crate::util::NULL_LSN.as_u64();
        self.current_index = -1;
    ...
    fn update_bin_pin(
        ...
        if let Some(old_arc) = self.current_bin_arc.take() {
            crate::tree::Tree::unpin_bin(&old_arc);
        }
        // Pin new BIN.
        if let Some(ref new_arc) = new_bin {
            crate::tree::Tree::pin_bin(new_arc);
        }
...
mod tests {
    use super::*;
    use crate::dbi::{DatabaseConfig, DatabaseId, DbType};
