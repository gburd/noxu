...
//!   [`crate::persist::evolve::ClassCatalog`] alongside the entity databases.
...
use crate::persist::entity::{Entity, PrimaryKey};
use crate::persist::error::{PersistError, Result};
use crate::persist::evolve::catalog::ClassCatalog;
use crate::persist::evolve::envelope;
use crate::persist::evolve::evolve_config::EvolveConfig;
use crate::persist::evolve::mutations::Mutations;
use crate::persist::evolve::stats::EvolveStats;
use crate::persist::primary_index::PrimaryIndex;
use crate::persist::secondary_index::SecondaryIndex;
use crate::persist::store_config::StoreConfig;
...
///
/// ```ignore
/// use crate::persist::{EntityStore, StoreConfig, PrimaryIndex, Entity, PrimaryKey};
...
impl<'env> EntityStore<'env> {
    ...
    pub fn open(env: &'env Environment, config: StoreConfig) -> Result<Self> {
        if !env.is_valid() {
            return Err(PersistError::DatabaseError(
                crate::db::NoxuError::EnvironmentClosed,
            ));
        }
    ...
    /// [`crate::persist::evolve::Mutations`] set was attached to the
        ...
        match persisted {
            ...
            Some(entry) => {
                if self.config.read_only {
                    return Err(PersistError::DatabaseError(
                        crate::db::NoxuError::OperationNotAllowed(format!(
...
mod tests {
    use super::*;
    use crate::persist::entity::Entity;
    use crate::persist::entity_serializer::EntitySerializer;
    ...
    fn test_evolve_empty_mutations_streams_records_no_converted() {
        ...
        let mutations = crate::persist::evolve::Mutations::new();
        let evolve_cfg = crate::persist::evolve::EvolveConfig::new();
    ...
    fn test_evolve_converter_transforms_records() {
        use crate::persist::evolve::{Converter, EvolveConfig, Mutations};
    ...
    fn test_evolve_deleter_removes_records() {
        use crate::persist::evolve::{Deleter, EvolveConfig, Mutations};
    ...
    fn test_evolve_on_closed_store_returns_error() {
        use crate::persist::evolve::{EvolveConfig, Mutations};
