...
use crate::tree::entry_states::DIRTY_BIT;
use crate::tree::error::TreeError;
use crate::tree::key::{create_key_prefix, get_key_prefix_length};
use hashbrown::HashSet;
use crate::util::{Lsn, NULL_LSN, Vlsn};
...
impl InNode {
    ...
    pub fn is_entry_embedded_ln(&self, index: usize) -> bool {
        if let Some(&state) = self.states.get(index) {
            state & crate::tree::entry_states::EMBEDDED_LN_BIT != 0
        } else {
            false
        }
    }
    ...
    pub fn is_entry_known_deleted(&self, index: usize) -> bool {
        self.states
            .get(index)
            .is_some_and(|&s| s & crate::tree::entry_states::KNOWN_DELETED_BIT != 0)
    }

    pub fn is_entry_pending_deleted(&self, index: usize) -> bool {
        self.states
            .get(index)
            .is_some_and(|&s| s & crate::tree::entry_states::PENDING_DELETED_BIT != 0)
    }
    ...
    pub fn set_known_deleted(&mut self, index: usize) {
        if let Some(s) = self.states.get_mut(index) {
            *s |= crate::tree::entry_states::KNOWN_DELETED_BIT;
            *s &= !crate::tree::entry_states::PENDING_DELETED_BIT;
            *s |= DIRTY_BIT;
        }
    }
    ...
    pub fn is_valid_for_delete(&self) -> bool {
        ...
        self.states
            .iter()
            .all(|&s| s & crate::tree::entry_states::KNOWN_DELETED_BIT != 0)
    }
...
impl Bin {
    ...
    pub fn is_defunct(&self, index: usize) -> bool {
        ...
        if let Some(ref expirations) = self.slot_expirations
            ...
            && crate::util::ttl::is_expired(exp, true)
        {
            return true;
        }
        false
    ...
    pub fn evict_lns(
        &mut self,
        log_manager: Option<&crate::log::LogManager>,
    ) -> usize {
        if self.has_cursors() {
            return 0;
        }
    ...
    pub fn evict_ln(
        ...
        {
            let entry = crate::log::entry::LnLogEntry::new(
                self.inner.db_id,
                None,
                crate::util::NULL_LSN,
                false,
            ...
            if let Ok(new_lsn) = lm.log(
                crate::log::LogEntryType::InsertLN,
                &buf,
                crate::log::Provisional::No,
                false,
                false,
            ) {
        ...
        if let Some(state) = self.inner.states.get_mut(index) {
            *state &= !crate::tree::entry_states::EMBEDDED_LN_BIT;
        }
...
mod tests {
    ...
    fn test_compress_removes_known_deleted_slots() {
        ...
        bin.inner.states[1] = crate::tree::entry_states::KNOWN_DELETED_BIT;
    ...
    fn test_compress_all_deleted_leaves_empty_bin() {
        ...
        bin.inner.states[0] = crate::tree::entry_states::KNOWN_DELETED_BIT;
        bin.inner.states[1] = crate::tree::entry_states::KNOWN_DELETED_BIT;
    ...
    fn test_in_node_is_valid_for_delete_not_all_deleted() {
        ...
        node.insert_entry(
            b"k2".to_vec(),
            Lsn::from_u64(2),
            crate::tree::entry_states::KNOWN_DELETED_BIT,
        )
        .unwrap();
        assert!(!node.is_valid_for_delete());
    }
    ...
    fn test_je_create_key_prefix_semantics() {
        use crate::tree::key::create_key_prefix;
