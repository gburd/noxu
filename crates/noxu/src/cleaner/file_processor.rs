...
use crate::cleaner::LnInfo;
use crate::cleaner::cleaner_stat::CleanerStats;
use bytes::BytesMut;
use crate::log::LogManager;
use crate::txn::{LockManager, LockType, TxnError};
use crate::util::Lsn;
...
fn write_migration_ln(
    ...
    use crate::log::{LogEntryType, Provisional, entry::LnLogEntry};
...
/// Real `TreeLookup` implementation backed by a shared `crate::tree::Tree`.
///
...
///
/// # M-5 fix
/// Changed from `RefCell<crate::tree::Tree>` (owned) to
/// `Arc<RwLock<crate::tree::Tree>>` (shared) so the environment's single
...
impl TreeLookup for RealTreeLookup {
    ...
    fn lookup_in(
            ...
            tree_guard.get_parent_in_for_child_in(node_id_u64)
        {
            ...
            // Step 3 — compare LSNs.
            if node_lsn == crate::util::NULL_LSN {
        ...
        if let Some(root) = root_arc_opt {
            ...
            if root_node_id == node_id_u64 {
                ...
                if root_lsn == crate::util::NULL_LSN || root_lsn != log_lsn {
                    return InLookupResult::Obsolete;
                }
...
impl RealTreeLookup {
    ...
    pub(crate) fn get_slot_lsn_from_root(
        root: Option<
            std::sync::Arc<crate::tree::NodeRwLock<crate::tree::TreeNode>>,
        >,
        key: &[u8],
    ) -> Option<Lsn> {
        let arc = root?;
    ...
    fn find_bin_entry_lsn(
        node_arc: &std::sync::Arc<crate::tree::NodeRwLock<crate::tree::TreeNode>>,
        key: &[u8],
    ) -> Option<Lsn> {
        use crate::tree::TreeNode;
...
/// Thread-safe `TreeLookup` implementation backed by a shared `crate::tree::Tree`.
///
...
pub struct SharedTreeLookup {
    ...
    extra_trees: HashMap<i64, Arc<RwLock<crate::tree::Tree>>>,
}
...
impl SharedTreeLookup {
    ...
    fn resolve_tree(&self, db_id: i64) -> &Arc<RwLock<crate::tree::Tree>> {
...
impl FileProcessor {
    ...
    pub fn process_file<T: TreeLookup>(
        &self,
        file_number: u32,
        _file_summary: &crate::cleaner::FileSummary,
        entries: &[LogEntry],
        tree: &T,
        ...
        for entry in entries {
            ...
            match &entry.entry_type {
                ...
                LogEntryType::Ln {
                    ...
                    let info = crate::cleaner::LnInfo::new(
                        lsn,
                        *db_id,
                        key.clone(),
                        *entry_size,
    ...
    pub fn process_file_no_entries(
        &self,
        file_number: u32,
        file_summary: &crate::cleaner::FileSummary,
    ) -> Result<FileProcessResult, String> {
    ...
    pub fn process_found_ln<T: TreeLookup>(
        ...
        // therefore obsolete.
        if tree_lsn == crate::util::NULL_LSN {
...
mod tests {
    ...
    impl TreeLookup for NullLsnTree {
        fn lookup_parent_bin(
            ...
            BinLookupResult::Found { tree_lsn: crate::util::NULL_LSN }
        }
    ...
    fn test_process_file_empty_entries() {
        ...
        let summary = crate::cleaner::FileSummary::new();
    ...
    fn make_tree_with_key(key: &[u8], lsn: Lsn) -> crate::tree::Tree {
        let tree = crate::tree::Tree::new(1, 128);
