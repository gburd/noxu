...
use crate::cleaner::FileSelector;
use crate::cleaner::cleaner_stat::CleanerStats;
use crate::cleaner::file_processor::{
    FileProcessResult, FileProcessor, LogEntry, LogEntryType, SharedTreeLookup,
};
use crate::cleaner::file_protector::FileProtector;
use crate::cleaner::throttle::CleanerThrottle;
use crate::log::{
...
pub struct Cleaner {
    ...
    tree: Option<Arc<RwLock<crate::tree::Tree>>>,
    ...
    lock_manager: Option<Arc<crate::txn::LockManager>>,
    ...
    extra_trees:
        Arc<std::sync::Mutex<HashMap<i64, Arc<RwLock<crate::tree::Tree>>>>>,
...
impl Cleaner {
    ...
    pub fn with_file_manager_and_tree(
        ...
        min_age: u64,
        file_manager: Arc<FileManager>,
        tree: Arc<RwLock<crate::tree::Tree>>,
    ...
    pub fn with_file_manager_tree_and_lock_manager(
        ...
        tree: Arc<RwLock<crate::tree::Tree>>,
        log_manager: Arc<LogManager>,
        lock_manager: Arc<crate::txn::LockManager>,
    ) -> Self {
        Self {
            file_selector: Mutex::new(FileSelector::new()),
            file_protector: FileProtector::new(),
    ...
    pub fn with_tree_registry(
        mut self,
        registry: Arc<
            std::sync::Mutex<HashMap<i64, Arc<RwLock<crate::tree::Tree>>>>,
        >,
    ) -> Self {
    ...
    fn process_single_file(
        ...
        let file_summary = match &self.file_manager {
            None => crate::cleaner::FileSummary::new(),
    ...
    fn decode_ln_entries_from_file(
        ...
        while offset < file_len {
            ...
            let lsn = crate::util::Lsn::new(file_number, file_offset);
                ...
                4 | 6 => {
                    ...
                    let (key, db_id): (Vec<u8>, i64) = if item_size > 0
                        && fm
                            ...
                            .is_ok()
                    {
                        use crate::log::entry::LnLogEntry;
    ...
    fn scan_file_summary(
        ...
        fm: &Arc<FileManager>,
        file_number: u32,
    ) -> crate::cleaner::FileSummary {
        let mut summary = crate::cleaner::FileSummary::new();
    ...
    pub fn after_checkpoint(
        &self,
        state: &crate::cleaner::file_selector::CheckpointStartCleanerState,
    ) {
...
mod tests {
    ...
    fn test_do_clean_file_protected_stays_pending() {
        ...
        assert_eq!(
            status,
            Some(crate::cleaner::FileStatus::Cleaned),
    ...
    fn make_fm_with_entries(
        dir: &std::path::Path,
    ) -> Arc<crate::log::FileManager> {
        use bytes::BytesMut;
        use crate::log::entry::TxnEndEntry;
        use crate::log::{FileManager, LogEntryType, LogManager, Provisional};
        use crate::util::{NULL_LSN, NULL_VLSN};
    ...
    fn test_with_file_manager_constructor() {
        let dir = tempfile::TempDir::new().unwrap();
        let fm = Arc::new(
            crate::log::FileManager::new(dir.path(), false, 10_000_000, 10)
                .unwrap(),
        );
    ...
    fn make_fm_and_lm(
        dir: &std::path::Path,
    ) -> (Arc<crate::log::FileManager>, Arc<crate::log::LogManager>) {
        use crate::log::{FileManager, LogManager};
    ...
    fn test_with_file_manager_and_tree_constructor() {
        ...
        let tree = Arc::new(RwLock::new(crate::tree::Tree::new(1, 128)));
    ...
    fn test_process_single_file_with_tree_migrates_live_ln() {
        ...
        use crate::log::file_header::FILE_HEADER_SIZE;
        ...
        let crc = crate::log::ChecksumValidator::compute_range(
            &hdr,
            4,
            MIN_HEADER_SIZE - 4,
        );
