...
impl TxnManager {
    ...
    /// call ([`crate::txn::Txn::new_auto`]).  The auto-txn:
    ///
    ...
    ///   explicit `Txn`, so [`crate::txn::Txn::abort`] can release the locks
    ...
    ///   when committed via [`crate::txn::Txn::commit_with_durability`], but
    ...
    /// environment.  Creates a [`crate::txn::group_commit::GroupCommitMaster`]
    /// with default configuration and stores it.
    ///
    /// (extended fork).
    pub fn setup_group_commit_master(&self) {
        use crate::txn::group_commit::GroupCommitMaster;
    ...
    pub fn setup_group_commit_replica(&self) {
        use crate::txn::group_commit::GroupCommitReplica;
