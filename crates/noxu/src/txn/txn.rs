...
use crate::txn::group_commit::GroupCommit;
use crate::txn::txn_abort::TxnAbort;
use crate::txn::txn_commit::TxnCommit;
use crate::txn::txn_state::TxnState;
use crate::txn::{
...
/// This `Txn` was allocated by [`crate::txn::TxnManager::begin_auto_txn`] to wrap
...
impl Txn {
    ...
    ///
    /// Returned by [`crate::txn::TxnManager::begin_auto_txn`] to wrap a single
    ...
    /// [`crate::txn::TxnManager::begin_auto_txn`].
    ///
    /// Used by [`Self::commit_with_durability`] and [`Self::abort`] to skip
    /// the `TxnCommit` / `TxnAbort` WAL entry, and by
    /// [`crate::txn::LockManager::register_locker_label`] callers to label the
    ...
    /// public `crate::db::Transaction::lock_counts()` accessor needs a
    ...
    ///
    /// Used by the higher-level `crate::db::Transaction::resolved_*_after_prepare`
    ...
    ///   `crate::xa::Xid` being prepared.  Encoded the same way the XA
    ...
    pub fn prepare(
        ...
        let timestamp_ms = std::time::SystemTime::now()
            ...
            .unwrap_or_default()
            .as_millis() as u64;
        let entry = crate::log::entry::TxnPrepareEntry::new(
            self.id,
            timestamp_ms,
            self.first_lsn,
            self.last_lsn,
