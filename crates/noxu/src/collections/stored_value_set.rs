...
use crate::db::{Database, OperationStatus, Transaction};

use crate::collections::error::Result;
use crate::collections::stored_iterator::StoredIterator;
    ...
    VB: EntryBinding<V>,
{
        ...
        V: PartialEq,
    {
        ...
        let mut key = crate::db::DatabaseEntry::new();
        let mut data = crate::db::DatabaseEntry::new();
        let mut status =
            cursor.get(&mut key, &mut data, crate::db::Get::First, None)?;
        let mut found = false;
        while matches!(status, OperationStatus::Success) {
            let v = self.value_binding.entry_to_object(&data).map_err(|e| {
                crate::collections::error::CollectionError::BindingError(e.to_string())
            })?;
            if &v == value {
                found = true;
                break;
            ...
            status =
                cursor.get(&mut key, &mut data, crate::db::Get::Next, None)?;
        }
        cursor.close()?;
        Ok(found)
    }
    ...
    pub fn iter(&self, txn: Option<&Transaction>) -> Result<StoredIterator<V>> {
        use crate::collections::internal::{ScanDirection, StartKey, scan_records};
        use crate::bind::ByteArrayBinding;
...
mod tests {
    use super::*;
    use crate::bind::{IntBinding, StringBinding};
    use crate::db::{DatabaseConfig, Environment, EnvironmentConfig};
    use tempfile::TempDir;

    use crate::collections::stored_map::StoredMap;
