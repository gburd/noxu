    ...
    VB: EntryBinding<V>,
{
    ...
    pub fn higher_key(
        ...
        let mut k_buf = crate::db::DatabaseEntry::new();
        let mut d_buf = crate::db::DatabaseEntry::new();
        let mut status =
            cursor.get(&mut k_buf, &mut d_buf, crate::db::Get::First, None)?;
        let mut result: Option<K> = None;
        while matches!(status, crate::db::OperationStatus::Success) {
            ...
            status =
                cursor.get(&mut k_buf, &mut d_buf, crate::db::Get::Next, None)?;
        }
        cursor.close()?;
        Ok(result)
    }
