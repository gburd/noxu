...
//! This is the BDB-JE shape, and it matches `crate::db::Database` and
//! `crate::db::SecondaryDatabase` so a typed `StoredMap` composes
//! cleanly with the rest of the engine.

use std::marker::PhantomData;

use crate::bind::EntryBinding;
use crate::db::{Database, Get, OperationStatus, Transaction};

use crate::collections::error::{CollectionError, Result};
use crate::collections::internal::{
...
/// `crate::db::Database` / `SecondaryDatabase` signature.
///
/// # Example
///
/// ```ignore
/// use crate::bind::{IntBinding, StringBinding};
/// use crate::collections::StoredMap;
///
    ...
    VB: EntryBinding<V>,
{
    ...
    pub fn get(&self, txn: Option<&Transaction>, key: &K) -> Result<Option<V>> {
        ...
        let mut data_entry = crate::db::DatabaseEntry::new();
...
mod tests {
    use super::*;
    use crate::bind::{ByteArrayBinding, IntBinding, StringBinding};
    use crate::db::{
    ...
    fn open_db(env: &Environment, name: &str) -> crate::db::Database {
