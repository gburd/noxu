    ...
    for s in &specs {
        ...
        helpers.push(quote! {
            #[doc = concat!(
                "Opens the `",
                #name_lit,
                "` persistent, transactional secondary index against the \
                 supplied entity store and primary index. \
                 Auto-generated by `#[derive(SecondaryKey)]`.\n\n\
                 The returned `SecondaryIndex` is backed by a real \
                 `noxu_db::SecondaryDatabase` maintained inside the active \
                 transaction — aborting a txn rolls the primary write and \
                 the secondary update back together.",
            )]
            pub fn #helper_name<'__store, __S>(
                store: &mut #krate::EntityStore<'__store>,
                primary: &mut #krate::PrimaryIndex<#pk_ty, Self>,
                serializer: ::std::sync::Arc<__S>,
            ) -> #krate::Result<#krate::SecondaryIndex<#sk_ty, #pk_ty, Self>>
            where
                __S: #krate::EntitySerializer<Self>
                    + ::std::marker::Send
                    + ::std::marker::Sync
                    + 'static,
            {
                store.open_secondary_index(
                    primary,
                    #name_lit,
                    serializer,
                    #extractor,
