...
mod tests {
    ...
    ///
    /// IGNORED: this test HANGS. It is not slow — it deadlocks. `run_as_leader`
    /// stamps the batch in arrival order, which puts the leader first, so the
    /// leader holds its own log-buffer `write_pin_count` pin (taken in
    /// `LogBuffer::allocate`, released only by its own `LogBufferSegment::put`
    /// after `run_as_leader` returns) across every other member's `assign`.
    /// The first later member that needs a buffer flip blocks forever in
    /// `LogBuffer::wait_for_zero_and_latch` on that undrainable pin, from
    /// inside the leader's own batch loop; all followers then spin in
    /// `wait_as_follower` indefinitely. Observed livelocked for >21h at ~200%
    /// CPU with 62 of 64 threads spinning as followers.
    ///
    /// Full root-cause analysis, gdb evidence, and a verified fix:
    /// `.agent/archived-audits/consolidation-array-deadlock-2026-09.md`.
    /// The bug is in production code (`consolidation.rs`), not in this test —
    /// the test is a correct reproducer. `#[ignore]`d rather than deleted so
    /// it can be un-ignored the moment the fix lands; leaving it un-ignored
    /// wedges `cargo test`/`cargo llvm-cov` for `noxu-log` indefinitely.
    #[test]
    #[ignore = "deadlocks: consolidation-array leader holds its own buffer pin \
