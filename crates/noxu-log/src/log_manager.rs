...
/// Per-entry request for the serial under-latch work.  Carries only the
/// LSN-independent metadata needed to assign a slot; the committer's owned
/// `entry_buf` (and its CRC finalisation) never crosses the latch — the
/// committer patches `prev_offset` + CRC after `assign_slot` hands
/// `SlotResult` back (JE `addPostMarshallingInfo` after the latch).
struct SlotRequest {
    entry_size: usize,
...
impl LogManager {
    ...
    /// state; the CALLER must guarantee single-threaded execution by holding
    /// `log_write_latch`.  Called
    ...
    fn log_internal(
        ...
        // Route the LSN-assign + prev_offset + buffer-slot reservation through
        // the `log_write_latch` mutex, one entry at a time in strict
        // LSN/arrival order.
        //
