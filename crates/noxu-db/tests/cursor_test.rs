...
fn cursor_put_overwrite_inserts_record() {
    ...
    let _s = cursor.put(&key, &val, Put::Overwrite).unwrap();
    cursor.close().unwrap();
...
fn cursor_put_no_overwrite_returns_key_exists() {
    ...
    let _s = cursor.put(&key, &v2, Put::NoOverwrite).unwrap();
}
...
fn cursor_put_current_updates_current_record() {
    ...
    let _s = cursor.put(&key, &new_val, Put::Current).unwrap();
    cursor.close().unwrap();
...
fn cursor_search_gte_oracle_brute_force_small_random() {
    ...
    for k in &keys {
        db.put(DatabaseEntry::from_bytes(k), DatabaseEntry::from_bytes(b"v"))
            .unwrap();
    }
...
// multiple BINs, and Hegel's shrinker minimises any failing key set.
...
fn cursor_next_dup_on_non_dup_db_returns_not_found() {
    ...
    db.put(DatabaseEntry::from_bytes(b"a"), DatabaseEntry::from_bytes(b"1"))
        .unwrap();
    db.put(DatabaseEntry::from_bytes(b"b"), DatabaseEntry::from_bytes(b"2"))
        .unwrap();
...
fn cursor_first_dup_and_last_dup_position_within_dup_set() {
    ...
    for v in [b"a", b"b", b"c"] {
        db.put(DatabaseEntry::from_bytes(b"k"), DatabaseEntry::from_bytes(v))
            .unwrap();
    }
...
fn cursor_with_txn_put_invisible_via_get_after_abort() {
    ...
    let mut out = DatabaseEntry::new();
    let status = db.get_into(None, &k, &mut out).unwrap();
    assert!(
        !status,
...
fn cursor_with_txn_get_takes_read_lock_via_locker() {
    ...
    let mut cursor = db.open_cursor_in(&txn_b, None).unwrap();
...
mod secondary_cursor_txn {
    ...
    fn sec_open_cursor_threads_txn_and_config() {
        ...
        let mut cursor = secondary.open_cursor_in(&txn, Some(&cfg)).unwrap();
        ...
        let mut cursor2 = secondary.open_cursor_in(&txn2, None).unwrap();
        ...
        let mut probe = secondary.open_cursor(None).unwrap();
        let s = probe
            .get_search_key(
