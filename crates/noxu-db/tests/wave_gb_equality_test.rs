//! Wave GB + P-2 equality-test harness.
//!
//! **Status**: P-2 scan-reduction SHIPPED (Wave GB proper fix).
//!
//! ## Design
//!
//! Each test writes a deterministic workload to a temp directory, forces
//! a clean close (which triggers a checkpoint), then reopens and asserts
//! that the recovered state matches the expected committed state.
//!
//! These tests validate the FULL P-2 recovery path:
//!   - Checkpoint writes root IN LSN in CkptEnd.
//!   - Recovery pre-populates BINs from the checkpoint tree-walk.
//!   - LN redo runs from `first_active_lsn` (not from LSN 0).
//!   - Uncommitted writes are correctly undone by the undo pass.
//!
//! The open-txn-at-crash correctness test lives in
//! `crash_recovery_test.rs::open_txn_spanning_checkpoint_recovers_correctly`
//! (requires SIGKILL infrastructure).
//!
//! ## Workloads
//!
...
// ---------------------------------------------------------------------------
// Open-transaction gap: NOW FIXED (P-2 proper)
// ---------------------------------------------------------------------------
...
#[test]
fn p2_committed_state_survives_checkpoint() {
    let dir = TempDir::new().unwrap();
