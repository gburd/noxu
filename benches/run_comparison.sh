...
lines.append("")
lines.append("The table above is the source of truth; read the JE/Noxu ratio column")
lines.append("(>1 = JE faster, <1 = Noxu faster) per workload and scale. Results vary by")
lines.append("storage (tmpfs vs NVMe), thread count, and JVM warmup — interpret from the")
lines.append("numbers in this run, not from baked-in commentary.")
