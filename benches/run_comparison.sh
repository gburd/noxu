...
lines.append("  • FSyncManager: both Noxu and JE coalesce N concurrent fsyncs into ~1 per cohort")
lines.append("    Noxu fsync counts now correctly exclude populate() phase (baseline subtracted)")
lines.append("    Remaining concurrent gap: JE FSyncManager coalesces more aggressively due to JVM thread model")
lines.append("  • Lock upgrade (READ→WRITE) via LockUpgrade::WritePromote — implemented, w09 100K cap removed")
lines.append("  • Abort undo implemented (fixed S28) — applies UndoRecords from WriteLockInfo to B-tree")
lines.append("  • B-tree merge/compress not implemented — tree only grows, never shrinks (minor)")
lines.append("  • w11 recovery speed is legitimate: full 3-phase (analysis+redo+undo), faster than JE due to no JVM overhead")
