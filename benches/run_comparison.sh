...
lines.append("")
lines.append("Noxu vs JE analysis (Session 29 — 100% structural fidelity):")
lines.append("  • LockManager blocks correctly (fixed S28) — concurrent gap is due to FSyncManager coalescing")
lines.append("    JE FSyncManager: N concurrent committers share 1 fsync; Noxu does 1 fsync/commit")
lines.append("    See w10_conc fsync counts: Noxu 200K fsyncs vs JE 52K for 4-writer/100K workload")
...
lines.append("  • w09_txn_multi/100K: Noxu hangs — get()+put() same key same txn hits lock upgrade path bug")
