...
fn main() {
    ...
    println!(
        "  dir={dir} (fs {fstype}) cache={}GiB records={records} val={value_size}B",
        cache / 1024 / 1024 / 1024
    );
    println!(
        "  sweep: 1,{step},{}.. up to {max_w} writers x {secs}s each",
        step * 2
    );
