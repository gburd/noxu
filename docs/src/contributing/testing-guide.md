...
Use `cargo nextest` for all LOCAL test runs. It is faster than `cargo test`,
shows cleaner output, and respects per-test timeouts from
`.config/nextest.toml`:
...
**CI does not use nextest today** — `.forgejo/workflows/test.yml` /
`.github/workflows/test.yml` run plain `cargo test --workspace [--all-features]
--no-fail-fast`, which runs the identical test set but without nextest's
per-test timeout enforcement (`.config/nextest.toml`'s slow-timeout currently
protects nothing in CI, only local runs). Prefer nextest locally for the
faster feedback loop and the timeout guard; do not assume CI has that guard
until the workflow is switched over.
...
| **`log_buffer` consolidation-array Log Write Latch (`shuttle_consolidation.rs`)** | **Green gate** | A separate mechanism from the segment pin/release above: the consolidation-array LWL (`consolidation.rs`, Aether VLDB'10 / Silo SOSP'13 / WiredTiger `log_slot.c`) is built on `AtomicPtr`/`AtomicBool`, not the raw `lock_api::RawMutex` that blocks the segment model — so it sidesteps that blocker entirely. Under the cfg these atomics resolve through `noxu_util::dst_sync::atomic` to shuttle-instrumented atomics, exploring the join / leader-handoff / follower-spin interleavings of the real funnel code. |
...
| **Replication sync: VLSN index + Paxos acceptor (`shuttle_rep_sync.rs`)** | **Green gate** (DST rep-sync coverage) | rep's async feeder/network/election-I/O loops are tokio and OUT of shuttle scope; the SYNC state machines are schedulable. Routes the VLSN index's `RwLock`s through `noxu_util::dst_sync_pl` and `PersistentAcceptorState`'s `Mutex`/atomics through `noxu_util::dst_sync` (default build byte-identical). **VLSN:** two writers (disjoint ranges) race a reader; asserts monotonic-latest, no-lost-mapping, no-torn-range (`noxu-spec` `vlsn_streaming`). **Acceptor:** proposers at distinct terms race `try_promise`/`try_accept`; asserts PromiseHonoured (`accepted_term <= promised_term`), promise-monotone, accept-implies-promise (`noxu-spec` `flexible_paxos`). **Not vacuous:** removing the bucket sort finds a lost VLSN mapping; removing the acceptor `flush_lock` finds a torn load-modify-store (`accepted_term > promised_term`). Surfaced a fidelity note (acceptor tracks the bare term, not the full JE `Proposal`) recorded in the archived audits. To keep noxu-db building under the cfg (rep dev-dep), the `Arc<RwLock<DatabaseImpl>>` seam was extended through `noxu-dbi::EnvironmentImpl` + noxu-db (import swaps only). |
