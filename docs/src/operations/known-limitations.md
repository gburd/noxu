...
| **Some cleaner/evictor tuning config parameters are not yet wired** | A few JE config parameters control features whose underlying model is not fully ported, so the parameters are accepted but ignored: `CLEANER_TWO_PASS_GAP` / `CLEANER_TWO_PASS_THRESHOLD` (JE's two-pass gate keys on a per-file min/max utilization *uncertainty band* that Noxu does not yet compute — Noxu uses a different `required_util` shortfall heuristic that is functionally a two-pass trigger); `BIN_DELTA_BLIND_OPS` / `BIN_DELTA_BLIND_PUTS` (blind-put-on-BIN-delta infra exists but production never enables it); `EVICTOR_MUTATE_BINS`, `EVICTOR_FORCED_YIELD`, `CLEANER_RMW_FIX`, `CLEANER_GRADUAL_EXPIRATION` (features not ported); `RESERVED_DISK` (disk-full reservation not implemented). | None needed for correctness — these are tuning knobs. `EVICTOR_USE_DIRTY_LRU` and `EVICTOR_EVICT_BYTES`/`EVICTOR_CRITICAL_PERCENTAGE` and `LOCK_N_LOCK_TABLES` were previously in this set and are now wired. |
| **`LockMode::Rmw` does not take a write lock (read-modify-write isolation)** | JE maps `LockMode.RMW` on a read to a WRITE lock so a subsequent update in the same transaction cannot deadlock and a concurrent writer is blocked at read time (`Cursor.java:5281`). Noxu accepts `LockMode::Rmw` but the cursor read path ignores the lock mode — an RMW read behaves like a plain read and does NOT block a concurrent writer. Found by the JE-fidelity test port (C7); the faithful `je_rmw_locking_test.rs` tests are `#[ignore]`d and will pass once RMW is wired. | For read-modify-write patterns that must serialize against concurrent writers, use SERIALIZABLE isolation or an explicit write (`put`) to take the write lock, rather than relying on `LockMode::Rmw`. |
...
| **Replication HA protocol is incomplete (election ranking, syncup, master transfer)** | Several JE replication protocol components are not yet ported: DTVLSN-based election ranking and `isAuthoritativeMaster` partition detection (a partitioned minority could wrongly elect a stale master), the `CommitFreezeLatch` (the freeze primitive is implemented but not yet wired into the replica replay / acceptor paths, so VLSN can still advance mid-election), the bilateral syncup matchpoint protocol (a diverged replica is not rolled back to a verified matchpoint — risk of divergent/lost writes), DTVLSN tracking + `DTVLSNFlusher`, and the master-transfer VLSN-progress event pump (`transferMaster()` currently does not actually change master). | **Do not rely on automatic failover / master election for correctness in production.** Replication is suitable for read-scaling and warm-standby with operator-supervised failover only. See the replication chapter; full HA election/syncup fidelity is in progress. |
