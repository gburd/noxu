...
**Branch**: `fix/gb-dbtree-recovery`
**Target**: v3.2.x (foundation) — scan-reduction deferred
**Outcome**: **ESCAPE HATCH APPLIED** — scan-reduction reverted; safe sub-parts shipped
...
2. **LSN-aware `redo_insert`** — recovery now correctly skips redo writes
   where the tree slot already holds a same-or-newer LSN.
