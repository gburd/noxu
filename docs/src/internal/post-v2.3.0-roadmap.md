...
| 11-S | v2.5.0 | UX + docs + cleanup from 2026-05 audit (H-1, H-3, H-5–H-8, Q-1, Q-2, Q-6, Q-7) | **done** — 5792 tests passing; Database::iter/range, abort lock-hold fix, log-alloc reduction, on-disk-format corrected, CursorImpl::search index bug fixed |
| 11-R | v3.0.0 | Breaking semantic correctness from 2026-05 audit (C-4, C-5, C-6, C-8, Q-3, Q-4) | **dispatched** — open_database txn rollback, BIN delta guards, recovery MapLN two-pass, SR9465/SR9752 investigation, verify()/evict_memory()/compress(), recovery test fidelity |
| 11-W | (audit) | Cross-feature / end-to-end second-pass audit | **dispatched** — replication×recovery, XA×recovery×rep, cleaner×checkpoint×recovery, secondary×txn×cleaner, eviction×checkpoint, config consistency, end-to-end scenario |
