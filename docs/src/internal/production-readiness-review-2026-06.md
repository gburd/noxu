...
- **St-C3 / St-H1 / St-H3** — **DONE**: St-H1/H3 docs corrected earlier; St-C3
  ...
  v2 backward-compat).
- **St-H6** — **CLOSED (two live bugs, both fixed).** (1) `Tree::split_child`
  hardcoded `expiration_in_hours: false` on the right-half sibling BIN,
  causing hours-granularity TTL records to silently expire on read (128/256
  keys missing in benchmark).  (2) `RecoveryManager::eligible_for_redo`
  applied an `after_ckpt_start` guard to non-transactional LNs; when the
...
| St-H2 | noxu-evictor | `real_node_size()` walks the entire tree per eviction decision (O(n) per node, O(n·batch) per batch) — no per-node in-memory-size tracking | **FIXED** (branch `fix/sth2-evictor-node-size`): new `find_node_full` does a **single** root-down walk per candidate extracting `NodeEvictionInfo`, byte size, and node `Arc` together.  `do_evict` uses a `RefCell<HashMap<u64,u64>>` size cache — `node_info_fn` deposits the size from the unified walk; `node_size_fn` drains it O(1) with no second tree walk.  Three prior separate recursive helpers (`find_node_info_recursive`, `find_node_size_recursive`, `find_node_arc_recursive`) removed and replaced by `find_node_full_recursive`.  Before: 2–3 O(tree) searches/Evict candidate.  After: 1 O(tree) search/candidate + O(1) size lookup.  Note: `flush_dirty_node_to_log` (dirty Evict nodes only) and `strip_lns_from_node` (PartialEvict BINs only) still call `find_node_arc` separately; both are minority paths and are candidates for a future locate-once pass.  Size formula byte-for-byte identical to prior `real_node_size`.  Tests: `test_find_node_full_bin_size_matches_formula`, `test_find_node_full_in_size_matches_formula`, `test_do_evict_bytes_matches_node_size_not_sentinel`. |
