...
| `docs/src/operations/benchmarks.md` | 189–268 | "Wave 11-B", "Wave 10-D", "Wave 11-C" in section titles and body |
...
| `docs/src/introduction.md` | Capability matrix preamble cross-references wave docs |
...
Dropped internal audit-finding citations ("Wave 2B closes audit
findings #1, #3, #4, #5, #11, and #12 from the May 2026 collections /
bind audit"). Kept the user-visible behavior change descriptions.

Inline wave-code annotations replaced with version labels:
- "v1.6 (Wave 2A) update" → "v1.6 update"
- "Wave 3-1" → "v2.0"
- "Sprint 1C / audit Finding 5" → removed, factual description kept
...
Described the feature factually in terms of what it does.
...
and from section title. Kept "v1.5", "v1.6" version labels.
Replaced "(matches the JE `EntityStore.evolve(EvolveConfig)` shape)"
with a factual description of the function.
...
Replaced "Wave 11-B", "Wave 10-D", "Wave 11-C" section titles and
body references with factual descriptions.
...
Replaced "production-grade, dependency-light" with
"dependency-light".

### Crate-level lib.rs and module docs

- `crates/noxu-collections/src/lib.rs`: removed "v1.6 API shape (Wave 2B)"
  heading, replaced "BDB-JE shape" with "same convention as
...
- `crates/noxu-collections/src/{internal,stored_iterator,stored_list,
  ...
  transaction_runner}.rs`: replaced "Wave 2B redesign (v1.6)." prefix
  with a one-sentence factual description.
...
- `crates/noxu-bind/tests/prop_tests.rs`: replaced `// Wave 11-E:` with
  a factual comment.
...
- `docs/src/internal/` — all wave documents are appropriate in this
  directory. The only new file written here is
  `docs/src/internal/wave-11-v-cleanup.md` (this document).
- `docs/src/maintainer/algorithms.md` — JE references here document
  algorithm provenance, which is appropriate for the maintainer audience.
...
- `docs/src/contributing/porting-guidelines.md` — JE/BDB-C references
  in the naming-conventions table are appropriate (they are the porting
  guide).
- `crates/noxu-persist-derive/src/lib.rs` — the JE annotation mapping
  table (`@Entity` → `#[derive(Entity)]`, etc.) is a useful design-
  rationale for the derive-macro author; kept.
- `crates/noxu-collections/src/lib.rs` migration section — kept the
  pointer to `docs/src/getting-started/migrating.md`; dropped the
  inline wave label.
- Test files whose `//! Wave N-X` header identifies the JE TCK port
  wave (e.g. `//! Wave 6 — Priority-3 JE TCK port.`) — these are
  internal test-registry notes, not user-facing docs. Left alone to
  preserve traceability.
- `AGENTS.md` — already task-oriented and direct. No wave/sprint
  language found in need of cleanup.
